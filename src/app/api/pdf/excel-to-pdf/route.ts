...
export async function POST(request: NextRequest) {
  console.log("Starting Excel to PDF conversion request");
  ...
  try {
    ...
    const acceptedTypes = [MIME_TYPES.xlsx, MIME_TYPES.xls];
    try {
    ...
    try {
      const settings = parseSettings(data.get("settings"), BaseSettingsSchema);
    ...
    // Send to ConvertAPI
    const sourceFormat = file.name.toLowerCase().endsWith('.xlsx') ? 'xlsx' : 'xls';
